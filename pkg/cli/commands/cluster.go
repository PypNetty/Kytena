...
and maps Kubernetes resources to Kytena workload models.`,
	...
	// Ajouter les flags
	cmd.Flags().StringVarP(&options.Namespace, "namespace", "n", "", "Namespace to filter workloads")

	cmd.Flags().BoolVar(&options.IncludePods, "include-pods", false, "Include pods in the workload discovery")

	cmd.Flags().BoolVar(&options.IncludeJobs, "include-jobs", false, "Include jobs in the workload discovery")
